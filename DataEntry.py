...
def bothDataEntry():
    c.execute("INSERT INTO both(Project, PathRow, User, Location) VALUES(?,?,?,?)",
              (projectname, pathrow1, username, locationname))
    conn.commit()
...
template =     ''' { "type" : "Feature",
        "properties" : { "fill" : "%s", "stroke" : "%s", "fill-opacity" : "0.8", "WRS2" : "%s", "Project" : "%s", "Author" : "%s", "Location" : "%s"}, "geometry" : %s},
    '''
...
for row in rawData:
    iter += 1
    if row[2].upper() == 'CMS_MEXICO' or projectname.upper() =='CMS MEXICO':
        fillcolor = '#fb8072'
    ...
    if iter >= 2 and iter != t:
        ...
        Author = row[4]
        Location = row[5]
        output += template % (fillcolor, linecolor, row[3], row[2], row[4], row[5], PR2)
    if iter == t:
        ...
        Author = row[4]
        Location = row[5]
        output += template2 % (fillcolor, linecolor, row[3], row[2], row[4], row[5], PR2)
